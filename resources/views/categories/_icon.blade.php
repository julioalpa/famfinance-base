{{--
  Partial: categories/_icon.blade.php
  Renderiza el chip de ícono de una categoría.
  Variables:
    $icon   — slug del ícono (string|null)
    $color  — color hex de la categoría (string|null)
    $type   — 'expense' | 'income' | 'both'
    $size   — 'xs' (18px) | 'sm' (24px, default) | 'md' (30px)
    $label  — nombre de la categoría para mostrar al lado (string|null, default null)
--}}
@php
...
// Tamaño del wrapper y del SVG
$wSize  = match($size) { 'xs' => 18, 'md' => 30, default => 24 };
$iSize  = match($size) { 'xs' => 10, 'md' => 16, default => 13 };
$radius = match($size) { 'xs' => 5,  'md' => 8,  default => 6  };
...
if ($color) {
    ...
    $bgColor     = match($type) { 'income' => 'rgba(45,216,112,0.12)',  'expense' => 'rgba(240,64,96,0.10)',  default => 'rgba(106,102,118,0.12)' };
    $borderColor = match($type) { 'income' => 'rgba(45,216,112,0.28)',  'expense' => 'rgba(240,64,96,0.22)',  default => 'rgba(106,102,118,0.25)' };
    $iconColor   = match($type) { 'income' => '#2dd870',                'expense' => '#f04060',               default => '#8a8699' };
}

// Biblioteca de íconos SVG (paths, 24x24 viewBox)
$ICONS = [
  // COMPRAS & COMIDA
...
$svgPaths = $ICONS[$icon] ?? $ICONS['other'];
@endphp

<div style="width:{{$wSize}}px;height:{{$wSize}}px;border-radius:{{$radius}}px;background:{{$bgColor}};border:1px solid {{$borderColor}};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
    <svg width="{{$iSize}}" height="{{$iSize}}" fill="none" stroke="{{$iconColor}}" stroke-width="1.6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        {!! $svgPaths !!}
    </svg>
</div>
@if($label)
    <span style="font-size:13px;font-weight:500;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $label }}</span>
@endif
